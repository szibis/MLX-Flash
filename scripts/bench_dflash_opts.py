...
def run_n_trials(runner, prompt, max_tokens, n=3, use_cache=True, label="baseline", **kwargs):
    """Run n trials and return median stats."""
    results = []
    for i in range(n):
        stats = run_trial(runner, prompt, max_tokens, use_cache=use_cache, label=label, **kwargs)
...
def print_result(label, stats, baseline_tok_s=None):
    ...
    print(
        f"  {label:30s} | {stats['tok_per_sec']:6.1f} tok/s | "
...
def main():
    ...
    baseline = run_n_trials(runner_base, args.prompt, args.max_tokens, n=args.trials, label="baseline-cached")
    print_result("BASELINE (flat, cached, b16)", baseline)
    base_tok_s = baseline["tok_per_sec"]

    # ── BASELINE: no cache ──
    nocache = run_n_trials(runner_base, args.prompt, args.max_tokens, n=args.trials, use_cache=False, label="no-cache")
    print_result("no-cache (reference)", nocache, base_tok_s)

    print()

    # ── OPT 1: block_size = 8 ──
    runner_b8 = DFlashRunner(model, tokenizer, drafter, config, inference_block_size=8)
    b8 = run_n_trials(runner_b8, args.prompt, args.max_tokens, n=args.trials, label="block-size-8")
    print_result("Opt1: block_size=8", b8, base_tok_s)

    # ── OPT 1b: block_size = 4 ──
    runner_b4 = DFlashRunner(model, tokenizer, drafter, config, inference_block_size=4)
    b4 = run_n_trials(runner_b4, args.prompt, args.max_tokens, n=args.trials, label="block-size-4")
    print_result("Opt1b: block_size=4", b4, base_tok_s)

    print()

    # ── OPT 2: hidden_dtype = float32 ──
    runner_f32 = DFlashRunner(model, tokenizer, drafter, config, hidden_dtype=mx.float32)
    f32 = run_n_trials(runner_f32, args.prompt, args.max_tokens, n=args.trials, label="hidden-float32")
    print_result("Opt2: hidden_dtype=float32", f32, base_tok_s)

    # ── OPT 2b: hidden_dtype = bfloat16 ──
    runner_bf16 = DFlashRunner(model, tokenizer, drafter, config, hidden_dtype=mx.bfloat16)
    bf16 = run_n_trials(runner_bf16, args.prompt, args.max_tokens, n=args.trials, label="hidden-bfloat16")
    print_result("Opt2b: hidden_dtype=bfloat16", bf16, base_tok_s)

    print()

    # ── OPT 3: mx.compile() drafter ──
    runner_compile = DFlashRunner(model, tokenizer, drafter, config, compile_drafter=True)
    compiled = run_n_trials(runner_compile, args.prompt, args.max_tokens, n=args.trials, label="compile-drafter")
    print_result("Opt3: compile_drafter=True", compiled, base_tok_s)

    print()

    # ── OPT 4: block_size=8 + compile + float32 (combo) ──
    runner_combo = DFlashRunner(
        model, tokenizer, drafter, config, inference_block_size=8, compile_drafter=True, hidden_dtype=mx.float32
    )
    combo = run_n_trials(runner_combo, args.prompt, args.max_tokens, n=args.trials, label="combo-b8-compile-f32")
    ...
    print(f"  Best: {best_name} at {best['tok_per_sec']:.1f} tok/s ({speedup:.2f}x vs baseline)")
    print(f"  Acceptance: {best['acceptance_rate']:.1%}, Tokens/step: {best['tokens_per_step']:.1f}")
    print("=" * 80)
