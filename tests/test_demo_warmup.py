...
class TestExpertFiles:
    def test_create_expert_files(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            expert_dir = create_expert_files(tmpdir, num_layers=2, num_experts=4, expert_size_bytes=1024)
            files = list(Path(expert_dir).rglob("*.bin"))
            assert len(files) == 8  # 2 layers * 4 experts

    def test_expert_file_sizes(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            expert_dir = create_expert_files(tmpdir, num_layers=1, num_experts=2, expert_size_bytes=512)
...
class TestWarmupSession:
    def test_session_records_metrics(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cache = LCPCache(str(expert_dir), capacity_bytes=4096, enable_dendritic=False, enable_skip_fallback=False)
            rng = np.random.default_rng(42)
