...
KNOWN_MODELS = [
    MoEModelInfo(
        ...
        0.5,
        "small",
        "Small MoE, great for testing. Fits on any Mac.",
    ),
    MoEModelInfo(
        ...
        2.0,
        "medium",
        "Strong general-purpose MoE. Fits on 32GB+ Macs.",
    ),
    MoEModelInfo(
        ...
        3.0,
        "large",
        "Powerful MoE. Needs 96GB+ for full RAM.",
    ),
    MoEModelInfo(
    ...
    MoEModelInfo(
        ...
        1.86,
        "huge",
        "Massive MoE. Needs SSD streaming.",
        available=False,
    ),
    MoEModelInfo(
        "N/A",
        ...
        2.0,
        "huge",
        "State-of-the-art MoE. 170GB at 4-bit.",
        available=False,
    ),
    MoEModelInfo(
        "N/A",
        ...
        1.86,
        "huge",
        "The Flash-MoE target. 209GB from SSD.",
        available=False,
    ),
]
...
def score_model(model: MoEModelInfo, hw: MacHardware) -> dict:
    ...
    est = estimate_performance(
        ...
        model.num_experts,
        model.top_k,
        model.expert_size_mb,
        model.gpu_layer_ms,
    )
    est_opt = estimate_performance(
        hw,
        model.size_gb,
        model.display_name,
        model.num_layers,
...
def print_model_browser(hw: MacHardware):
    ...
    for i, model in enumerate(KNOWN_MODELS):
        ...
        print(
            f"║ {run}{i + 1:>2d} {model.display_name:<30s} {model.size_gb:>3.0f}G {fits:>7s} {score['base_tps']:>5.1f} {score['optimized_tps']:>5.1f} {gain:>7s}  ║"
        )
    ...
    for model in KNOWN_MODELS:
        ...
        if score["needs_ssd"] and model.num_experts > 1:
            ...
            print(
                f"║    + Cache:   {score['optimized_tps']:>5.1f} tok/s  {b2:<20s}  {(score['speedup'] - 1) * 100:>+.0f}%    ║"
            )
            print("║                                                                      ║")
    ...
    if best:
        m = best[0]
        print("║  QUICK RUN: python -m mlx_flash_compress.model_browser --run 1      ║")
...
def run_model(model: MoEModelInfo, tokens: int = 100):
    ...
    subprocess.run(
        [sys.executable, "-m", "mlx_flash_compress.run", "--model", model.name, "--tokens", str(tokens)],
    )
